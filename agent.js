...
const titleRefinerAgent = new AgentTool(
  ...
  async (query) => {
    const prompt = ChatPromptTemplate.fromMessages([
      ["system", `You are a movie title expert. Your job is to:
...
const movieFetcherAgent = new AgentTool(
  ...
  async (query) => {
    const prompt = ChatPromptTemplate.fromMessages([
      ["system", `You are a movie data fetcher and enhancer agent. Your responsibilities include:
...
const fileWriterAgent = new AgentTool(
  ...
  async (query) => {
    const prompt = ChatPromptTemplate.fromMessages([
      ["system", `You are a file formatting specialist. Given movie data in JSON format, create a beautifully formatted movie summary with:
...
class MovieSummarizerBot {
  constructor() {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      ["system", `You are a movie summarizer orchestrator. Your job is to coordinate movie summarization tasks by calling the appropriate agents in sequence:

      1. For any movie title input, first call the title_refiner_agent to get the correct title
      2. Then call the movie_fetcher_agent with the refined title to get movie details
      3. Finally call the file_writer_agent to save the movie summary to a file

      Always use the available tools to complete the movie summarization process. 
      Process each step sequentially and pass the output of one agent as input to the next when needed.`],
      ["human", "{input}"],
      ["placeholder", "{agent_scratchpad}"]
    ]);
    
    const agent = createToolCallingAgent({ llm: model, tools: this.tools, prompt });
    this.executor = new AgentExecutor({ agent, tools: this.tools, verbose: false });
  }
  ...
  const ask = () => {
    rl.question('Enter movie title: ', async (input) => {
      ...
      try {
        ...
        console.log("Error occurred while processing movie. Please try again.\n");
